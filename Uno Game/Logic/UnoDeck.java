...
public class UnoDeck {
    ...
    public void reset() {
        ...
        UnoCard.Value[] values = new UnoCard.Value[]{UnoCard.Value.Wild, UnoCard.Value.WildDrawFour};
        for (UnoCard.Value value : values) {
            for (int i = 0; i < 4; i++) {
    ...
    // Replace the Deck with an ArrayList of UnoCard
    ...
    public UnoCard drawCard() throws IllegalArgumentException {
        if (isEmpty()) {
            throw new IllegalArgumentException("Can't draw a card since there are no cards in the Deck");
        }
    ...
    public UnoCard[] drawCard(int n) {
        ...
        if (n > cardsInDeck) {
            throw new IllegalArgumentException("Can't draw " + n + " cards since there are only " + cardsInDeck + " cards.");
        }
